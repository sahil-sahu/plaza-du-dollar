...
const Card1 = ({img, mrp, sp, name, uid}:{img:string; mrp:number; sp:number; name:string; uid:string;}) =>{
     return (
        <Link className="h-[400px]" href={'/product/'+uid}>
            <div className="rounded-xl h-full relative flex flex-col justify-between hover:border-my_green border-gray-300 ease-in-out duration-150 border-2 overflow-hidden">
            {sp < mrp ? (
            ...
            <div className="divide-solid divide-gray-400 p-4">
                <div className="text-lg">
                    <h4 className="font-bold line-clamp-3">{name}</h4>
                    {
                        sp < mrp ? (
